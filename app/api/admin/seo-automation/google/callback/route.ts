...
export async function GET(req: NextRequest) {
  ...
  try { const redirectUri = process.env.GOOGLE_SEARCH_CONSOLE_REDIRECT_URI || `${url.origin}/api/admin/seo-automation/google/callback`; await exchangeSearchConsoleCode(code, redirectUri, admin.user!.id); const response = NextResponse.redirect(new URL("/admin?tab=site_settings&seo=connected", url.origin)); response.cookies.set("seo_google_oauth_state", "", { maxAge: 0, path: "/" }); return response; }
