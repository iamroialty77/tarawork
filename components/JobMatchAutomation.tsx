...
type Recipient = { userId: string; name: string; email: string; jobTitle: string; company: string; score: number; matchedSkills: string[] };
  ...
  return <div className="absolute inset-0 z-20 overflow-y-auto bg-white">
    ...
    <div className="mx-auto max-w-5xl p-5 md:p-8">
      <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div><p className="font-black text-slate-800">Daily intelligent job matching</p><p className="text-xs font-medium text-slate-500">Each freelancer receives only their highest-scoring unsent job match.</p></div>
      ...
      <p className="mt-2 text-xs font-medium text-slate-400">Available fields: {"{{name}}"}, {"{{job_title}}"}, {"{{company}}"}, {"{{match_score}}"}, {"{{matched_skills}}"}, {"{{job_url}}"}.</p>
