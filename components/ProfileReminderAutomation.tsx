...
type Recipient = { id: string; name: string; email: string; role: string; completion: number };
...
export default function ProfileReminderAutomation({ close }: { close: () => void }) {
  ...
  return <div className="absolute inset-0 z-20 overflow-y-auto bg-white">
    ...
    <div className="w-full p-4 md:p-5">
      ...
      <p className="mt-2 text-xs font-medium text-slate-400">Available fields: {"{{name}}"}, {"{{role}}"}, {"{{completion}}"}, {"{{profile_url}}"}.</p>
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        {recipients.length > 0 && <div className="mt-3 max-h-48 divide-y divide-slate-200 overflow-y-auto rounded-xl border border-slate-200 bg-white">{recipients.map((recipient) => <div key={recipient.id} className="flex items-center gap-3 px-3 py-2.5 text-xs"><div className="min-w-0 flex-1"><p className="truncate font-black text-slate-700">{recipient.name}</p><p className="truncate text-slate-400">{recipient.email} · {recipient.role}</p></div><span className="rounded-lg bg-amber-50 px-2 py-1 font-black text-amber-700">{recipient.completion}%</span></div>)}</div>}
      </div>
