...
export default function PublicInfoPage({ eyebrow, title, introduction, sections, updated = "July 20, 2026" }: { eyebrow: string; title: string; introduction: string; sections: InfoSection[]; updated?: string }) {
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4"><Link href="/" className="flex items-center gap-3 font-black"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-white"><ShieldCheck className="h-5 w-5" /></span>TaraWork.online</Link><Link href="/contact" className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50"><Mail className="h-4 w-4" />Contact</Link></div></header>
