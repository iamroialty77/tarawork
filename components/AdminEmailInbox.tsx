...
import { Archive, Bot, BriefcaseBusiness, ChevronLeft, File, FilePenLine, HardDrive, Inbox, Mail, Paperclip, PenLine, RefreshCw, RotateCcw, Search, Send, Trash2, Users, X } from "lucide-react";
...
export default function AdminEmailInbox({ messages, refresh, reply }: { messages: any[]; refresh: () => void; reply: (id: string, body: string) => Promise<void> }) {
  ...
  return <div className="relative grid min-h-[700px] grid-cols-1 overflow-hidden border-y border-slate-200 bg-white lg:grid-cols-[230px_360px_minmax(0,1fr)]">
    <aside className="border-b border-slate-200 bg-slate-50/80 p-4 lg:border-b-0 lg:border-r">
      ...
      <button onClick={() => setShowAutomation(true)} className="mb-2 flex w-full items-center justify-center gap-2 rounded-2xl border border-violet-200 bg-white px-4 py-3 text-sm font-black text-violet-700 transition hover:bg-violet-50"><Bot className="h-4 w-4" />Profile reminder bot</button>
      <button onClick={() => setShowJobMatches(true)} className="mb-5 flex w-full items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-white px-4 py-3 text-sm font-black text-emerald-700 transition hover:bg-emerald-50"><BriefcaseBusiness className="h-4 w-4" />Job match bot</button>
    ...
    {showAutomation && <ProfileReminderAutomation close={() => setShowAutomation(false)} />}
    {showJobMatches && <JobMatchAutomation close={() => setShowJobMatches(false)} />}
  </div>;
}
