...
CREATE TABLE IF NOT EXISTS public.rss_automation_configs (
    id TEXT PRIMARY KEY DEFAULT 'primary' CHECK (id = 'primary'), enabled BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS public.rss_automation_runs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(), trigger_type TEXT NOT NULL CHECK (trigger_type IN ('manual', 'cron')),
    status TEXT NOT NULL DEFAULT 'running' CHECK (status IN ('running', 'completed', 'failed')),
    inserted_count INTEGER NOT NULL DEFAULT 0, duplicate_count INTEGER NOT NULL DEFAULT 0, expired_count INTEGER NOT NULL DEFAULT 0,
    errors JSONB NOT NULL DEFAULT '[]'::jsonb, started_by UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
